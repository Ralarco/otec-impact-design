...
function HomePage() {
  return <div className="min-h-screen bg-background text-foreground">
    ...
    <main>
      ...
      <section id="sence" className="relative overflow-hidden bg-primary py-20 text-primary-foreground md:py-28"><div className="absolute right-0 top-0 h-full w-1/3 bg-secondary/10 [clip-path:polygon(35%_0,100%_0,100%_100%,0_100%)]"/><div className="section-shell relative grid items-center gap-14 lg:grid-cols-[1.1fr_.9fr]"><div><SectionHeading eyebrow="Franquicia tributaria" title="Haz que tu inversión en personas llegue más lejos" /><p className="max-w-2xl text-base leading-7 text-primary-foreground/75">Te orientamos para aprovechar la franquicia tributaria SENCE y organizar un plan de capacitación coherente con las necesidades de tu empresa.</p><div className="mt-8 grid gap-4 sm:grid-cols-2">{["Detección de necesidades", "Diseño del plan formativo", "Coordinación de cursos", "Acompañamiento en el proceso"].map(item => <div key={item} className="flex items-center gap-3 text-sm font-semibold"><CheckCircle2 className="size-5 shrink-0 text-accent" />{item}</div>)}</div><a href="#contacto" className="mt-9 inline-flex h-12 items-center border border-primary-foreground/40 px-5 text-sm font-bold transition-colors hover:bg-primary-foreground hover:text-primary">Conversemos sobre SENCE <ArrowRight className="ml-2 size-4" /></a></div><div className="border-l border-primary-foreground/20 pl-8"><p className="font-display text-3xl font-bold uppercase leading-tight md:text-4xl">“Capacitar no es solo cumplir. Es preparar a tu equipo para tomar mejores decisiones.”</p><div className="mt-7 h-1 w-20 bg-accent" /></div></div></section>
